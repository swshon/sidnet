...
tst_tests = []
spkdict={}
for idx,line in enumerate(open('data/'+TEST_SET_NAME+'/utt2spk').readlines()):
    spkdict[line.split()[0]] = idx

for line in open('/data/sls/SID/Corpora/voxceleb1/voxceleb1_test.txt').readlines():
    row = line.rstrip().split()
    tst_trials.append(int(row[0]))
    tst_enrolls_idx.append(spkdict[row[1].split('.wav')[0]])
    tst_tests_idx.append(spkdict[row[2].split('.wav')[0]])
