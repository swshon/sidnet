...
#     for iter in range(10):
    ...
    for step in range(RESUME_STARTPOINT,MAX_ITER):
        ...
        if step % EERTEST_INTERVAL ==0 and step>=RESUME_STARTPOINT:
            ...
            embeddings = []
            for iter in range(len(test_list)):
                eb = emnet_validation.end_points[args.main_scope+'/'+EMBEDDING_SCOPE].eval({test_feat_batch:[test_feats[iter]], test_label_batch:[test_labels[iter]], test_shape_batch:[test_shapes[iter]]})
