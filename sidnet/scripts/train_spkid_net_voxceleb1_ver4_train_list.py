...
parser.add_argument("--train_data", type=str, default='data/train_list', help="target data")
...
parser.add_argument("--momentum", type=float, default=0.9, help="maximum length for random sub-sampling")
...
for line in lines:
    ...
    data_name[row[0]]=int(row[1])

records_shuffle_list = []
for name in data_name.keys():
    for i in range(1, data_name[name]+1):
        records_shuffle_list.append(TFRECORDS_FOLDER+'/'+name+'_'+FEAT_TYPE+'.'+str(i)+'.tfrecords')
...
with tf.device('/cpu:0'):
    ...
    if args.optimizer=='adam':
        ...
        learning_rate = tf.train.exponential_decay(LEARNING_RATE, global_step, 32000, 0.98, staircase=True)
...
#     for iter in range(10):
    ...
    for step in range(RESUME_STARTPOINT,MAX_ITER):
        ...
        if args.cnn1d: # for 1-d CNN input (batch,feat_length,feat_dim)
            feats = feats[:,:,:new_input_length]
        else: # for 2-d CNN input (batch,feat_dim,feat_length,1(channel))
            feats = feats[:,:,:new_input_length,:]
